...
export function EvaluationsPage() {
  ...
  const reviewableTeams = useMemo(
    () => (teams.data || []).filter((team) => team.participation_mode === 'TEAM'),
    [teams.data],
  );
  useEffect(() => {
    if (reviewableTeams.length && !reviewableTeams.some((team) => team.team_id === teamId)) {
      setTeamId(reviewableTeams[0].team_id);
    }
  }, [reviewableTeams, teamId]);
  ...
  if (teams.loading || teams.error) return <PageState loading={teams.loading} error={teams.error} />;
  ...
  return <>
    ...
    <div className="evaluation-page-grid">
      <section className="content-card teammate-review-card">
        ...
        {!reviewableTeams.length ? <PageState empty="평가할 팀 활동이 없습니다." /> : <form onSubmit={submit}>
          <label>활동 선택<select aria-label="활동 선택" value={teamId || ''} onChange={(event) => setTeamId(Number(event.target.value))}>{reviewableTeams.map((team) => <option value={team.team_id} key={team.team_id}>{team.team_name}{team.activity_status === 'COMPLETED' ? ' · 완료' : ''}</option>)}</select></label>
          {members.loading || members.error ? <PageState loading={members.loading} error={members.error} /> : <label>팀원 선택<select aria-label="팀원 선택" value={memberId || ''} onChange={(event) => setMemberId(Number(event.target.value))}>{(members.data || []).filter((member) => member.user_id !== user?.id).map((member) => <option value={member.user_id} key={member.user_id}>{member.name} · {member.part || '역할 미정'}</option>)}</select></label>}
          {existing.error && <PageState error={existing.error} />}
          {memberId ? <><div className="rating-options">{ratingOptions.map(({ value, label, icon: Icon }) => <button type="button" className={rating === value ? 'active' : ''} aria-pressed={rating === value} onClick={() => setRating(value)} key={value}><Icon /><strong>{label}</strong></button>)}</div><label>평가 코멘트<textarea rows={6} maxLength={500} value={comment} onChange={(event) => setComment(event.target.value)} placeholder="함께 활동하며 좋았던 점과 도움이 될 의견을 남겨주세요." /><small className="field-counter">{comment.length}/500</small></label><button className="primary-button" disabled={saving || existing.loading || !rating || !comment.trim()}>{saving ? '저장 중…' : existing.data?.existingReview ? '평가 수정' : '평가 저장'}</button></> : !members.loading && <PageState empty="평가할 다른 팀원이 없습니다." />}
      ...
      <section className="content-card received-review-card">
        ...
        {summary.error ? <PageState error={summary.error} /> : <div className="evaluation-summary">{ratingOptions.map(({ value, label, icon: Icon }) => <div key={value}><span><Icon /></span><strong>{Number(evaluation?.[`review_${value}` as keyof typeof evaluation] || 0)}</strong><small>{label}</small></div>)}</div>}
        <h4><MessageSquareText /> 받은 평가 코멘트</h4>
        <div className="received-review-list">{received.loading || received.error ? <PageState loading={received.loading} error={received.error} /> : received.data?.reviews?.length ? received.data.reviews.map((review) => <article key={review.review_id}><header><strong>{review.activity_title}</strong><span>{ratingFrom(review) === 'high' ? '최고예요' : ratingFrom(review) === 'medium' ? '좋아요' : '아쉬워요'}</span></header><p>{review.comment || '코멘트가 없습니다.'}</p><small>{review.reviewer_name} · {new Date(review.created_at).toLocaleDateString('ko-KR')}</small></article>) : <PageState empty="아직 받은 평가 코멘트가 없습니다." />}</div>
      </section>
    </div>
  </>;
}
