...
export function ApplicationsPage() {
  ...
  return <>
    <PageTitle title="나의 지원" description="지원부터 팀 합류까지 진행 상황을 확인하세요." />
    <ApplicationNavigation />
    <PageState loading={result.loading} error={result.error} empty={!result.loading && !result.data?.length ? '지원한 모집글이 없습니다.' : undefined} />
    <div className="recruitment-list">{result.data?.map((item) => <Link to={`/matching/applications/${item.application_id}`} className="application-row" key={item.application_id}><StatusPill application={item} /><div><h3>{item.post_name}</h3><p>{item.activity_name}</p></div><ChevronRight /></Link>)}</div>
  </>;
}
