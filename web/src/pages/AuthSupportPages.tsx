...
export function RegisterPage() {
  ...
  return <AuthFrame>
    ...
    <form className="stack-form" onSubmit={submit}>
      ...
      <div className="form-grid two"><label>이름<input name="name" required /></label><label>학과<input name="department" /></label><label>학번<input name="studentId" /></label><label>생년월일<input name="birth" type="date" /></label><label>비밀번호<input name="password" type="password" minLength={10} maxLength={128} title="문자와 숫자를 포함해 10자 이상 입력하세요 (최대 72바이트)" required /></label><label>비밀번호 확인<input name="password_confirm" type="password" minLength={10} maxLength={128} title="문자와 숫자를 포함해 10자 이상 입력하세요 (최대 72바이트)" required /></label></div>
...
export function ForgotPasswordPage() {
  ...
  return <AuthFrame>
    ...
    {step === 'PASSWORD' && <form className="stack-form" onSubmit={confirm}><label>새 비밀번호<input name="password" type="password" minLength={10} maxLength={128} title="문자와 숫자를 포함해 10자 이상 입력하세요 (최대 72바이트)" required /></label><label>새 비밀번호 확인<input name="password_confirm" type="password" minLength={10} maxLength={128} title="문자와 숫자를 포함해 10자 이상 입력하세요 (최대 72바이트)" required /></label><button className="primary-button" disabled={busy}>비밀번호 변경</button></form>}
    {message && <div className="form-success"><CheckCircle2 />{message}</div>}{error && <div className="form-error">{error}</div>}
  </AuthFrame>;
}
