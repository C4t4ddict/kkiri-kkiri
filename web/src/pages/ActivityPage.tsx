...
import { ActivityProgress } from '../features/activity/ActivityProgress';
import type { GoalGroups } from '../features/activity/goalProgress';
...
const currentRanges = (today: string) => {
...
export function ActivityPage() {
  ...
  const ranges = useMemo(() => currentRanges(today), [today]);
  ...
  const goals = useAsync(async () => {
    if (!selectedId) return { teamId: selectedId, today, items: { 월간: [], 주간: [], 일일: [] } as GoalGroups };
    ...
    return { teamId: selectedId, today, items: { 월간: monthly, 주간: weekly, 일일: daily } };
  }, [selectedId, today]);
  const goalData = goals.data?.teamId === selectedId && goals.data?.today === today ? goals.data.items : null;
  ...
  const allGoals = useMemo(() => goalData ? [...goalData.월간, ...goalData.주간, ...goalData.일일] : [], [goalData]);
  ...
  const weekDays = useMemo(() => Array.from({ length: 7 }, (_, index) => {
    ...
    const items = (goalData?.일일 || []).filter((goal) => goal.scope_start_date.slice(0, 10) <= key && goal.scope_end_date.slice(0, 10) >= key);
    return { key, label: ['월', '화', '수', '목', '금', '토', '일'][index], date: date.getDate(), items, percent: progressOf(items), today: key === dateKey(new Date()) };
  }), [goalData?.일일, ranges.weekStart]);
  ...
  return <div className="activity-workspace-page">
    <header className="activity-page-header">
      ...
      <nav className="activity-header-actions" aria-label="활동 시작과 기록"><Link className="ghost-button" to="/activity/new"><Plus size={17} /> 우리 팀 활동 시작</Link><Link className="text-link" to="/activity/archive">완료한 활동·기록 <ChevronRight size={16} /></Link></nav>
    </header>
    ...
    {!active.data?.length ? <section className="activity-empty-panel"><UsersRound /><h2>우리 팀의 첫 활동을 시작해보세요</h2><p>이미 모인 팀은 바로 시작하고, 함께할 사람이 필요하면 팀을 찾아보세요.</p><div className="button-row"><Link className="primary-button" to="/activity/new">우리 팀으로 시작</Link><Link className="ghost-button" to="/matching">함께할 팀 찾기</Link><Link className="text-link" to="/curriculum">커리큘럼으로 시작</Link></div></section> : <div className="workspace-layout">
      ...
      <div className="workspace-main">
        <section className="workspace-hero">
          ...
          <ActivityProgress key={`${selectedId}:${today}`} groups={goalData} today={today} loading={goals.loading} error={goals.error} onRetry={goals.reload} />
        </section>

        <section className="activity-journey-launch"><div><h2>함께한 작업을 내 경험으로</h2><p>문서와 결과물을 연결하고, 내가 기여한 내용을 포트폴리오에 남기세요.</p></div><nav className="activity-journey-actions" aria-label="작업과 포트폴리오"><Link className="primary-button" to={`/activity/${selectedId}/work`}>공동 작업·기여 기록</Link><Link className="ghost-button" to={`/activity/${selectedId}/portfolio`}>내 포트폴리오</Link></nav></section>
        ...
        {selected?.participation_mode === 'TEAM' && <section className="selected-team-panel" aria-labelledby="selected-team-heading">
          ...
          {!members.loading && !members.error && Boolean(members.data?.length) && <div className="selected-team-members">
            {members.data?.map((member) => <article key={member.user_id}>
              <UserAvatar className="selected-team-avatar" user={member} />
