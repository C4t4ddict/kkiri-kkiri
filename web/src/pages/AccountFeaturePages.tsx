...
export function FavoritesPage() {
  ...
  return <><PageTitle title="관심 활동" description="저장해둔 공모전과 대외활동을 한곳에서 다시 확인하세요." /><PageState loading={result.loading} error={result.error} empty={!result.loading && !result.data?.length ? '관심 활동을 아직 저장하지 않았습니다.' : undefined} /><div className="activity-grid">{result.data?.map((item) => <ActivityCard item={item} key={item.activity_id} />)}</div></>;
}
...
export function ActivityArchivePage() {
  ...
  return <><PageTitle title="지난 활동" description="완료한 경험을 미니포트폴리오로 다시 꺼내보세요." /><PageState loading={result.loading} error={result.error} empty={!result.loading && !result.data?.length ? '아직 완료된 활동이 없습니다.' : undefined} /><div className="archive-grid">{result.data?.map((item) => <Link className="archive-card" to={`/mypage/archive/${item.portfolio_id}`} key={item.portfolio_id}><div className="archive-top"><span>{item.activity_type || '팀 활동'}</span><small>{item.archived_at?.slice(0, 10)} 저장</small></div><h3>{item.activity_name}</h3><p>{item.role || '역할 미정'} · {item.period || '기간 미정'}</p><div><CheckCircle2 /> 완료 작업 {item.completed_task_count || 0}개 <em>포트폴리오 보기</em></div></Link>)}</div></>;
}
...
{portfolio.team_id && <nav className="journey-nav" aria-label="완료한 활동 이어가기"><Link to={`/activity/${portfolio.team_id}/portfolio`}>내 기여·공유 포트폴리오</Link><Link to={`/activity/${portfolio.team_id}/work`}>작업·결과물 기록</Link><Link to={`/activity/${portfolio.team_id}/documents`}>활동 문서</Link><Link to={`/mypage/evaluations?team=${portfolio.team_id}`}>팀원 평가</Link></nav>}
