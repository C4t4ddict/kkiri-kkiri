...
export function FavoritesPage() {
  ...
  return <><PageTitle title="나의 관심 활동" description="저장해둔 공모전과 대외활동을 한곳에서 다시 확인하세요." /><PageState loading={result.loading} error={result.error} empty={!result.loading && !result.data?.length ? '관심 활동을 아직 저장하지 않았습니다.' : undefined} /><div className="activity-grid">{result.data?.map((item) => <ActivityCard item={item} key={item.activity_id} />)}</div></>;
}
...
export function AccountSettingsPage() {
  ...
  const [preferences, setPreferences] = useState<Record<string, boolean>>({ matching: true, activity: true, todo: true, notice: true });
  const preferenceResult = useAsync(
    () => user ? api<{ preferences: Record<string, boolean> }>('/api/me/notification-preferences') : Promise.resolve(null),
    [user?.id],
  );
  useEffect(() => {
    if (preferenceResult.data?.preferences) setPreferences(preferenceResult.data.preferences);
  ...
  const saveProfile = (event: FormEvent<HTMLFormElement>) => { event.preventDefault(); const form = new FormData(event.currentTarget);
    run(async () => { await api(`/api/user/${user.id}`, { method: 'PUT', body: JSON.stringify({ name: String(form.get('name') || ''), department: String(form.get('department') || ''), student_number: String(form.get('student_number') || ''), birth_date: String(form.get('birth_date') || '') || null, self_intro: String(form.get('self_intro') || '') }) }); await refreshUser(); }, '개인정보를 저장했습니다.');
  };
  ...
  const togglePreference = async (key: string) => {
    ...
    setError('');
    try {
      await api('/api/me/notification-preferences', { method: 'PUT', body: JSON.stringify(next) });
  ...
  return <><PageTitle title="계정 및 개인정보" description="프로필, 비밀번호, 알림 환경을 관리하세요." />{message && <div className="form-success"><CheckCircle2 />{message}</div>}{error && <div className="form-error">{error}</div>}
    <div className="settings-layout"><form className="content-card feature-form" onSubmit={saveProfile}><div className="feature-card-title"><Settings /><div><h2>개인정보</h2><p>서비스에서 사용하는 기본 정보입니다.</p></div></div><div className="account-profile-upload"><UserAvatar user={user} className="account-profile-preview" /><div><strong>프로필 사진</strong><p>JPG, PNG, WEBP 이미지로 변경할 수 있습니다.</p><label className="ghost-button"><ImagePlus /> 사진 변경<input type="file" accept="image/jpeg,image/png,image/webp" disabled={busy} onChange={(event) => { uploadProfile(event.target.files?.[0]); event.target.value = ''; }} /></label></div></div><div className="form-grid two"><label>이메일<input value={user.email} disabled /></label><label>이름<input name="name" defaultValue={user.name} required /></label><label>학과<input name="department" defaultValue={user.department} /></label><label>학번<input name="student_number" defaultValue={user.studentId} /></label><label>생년월일<input name="birth_date" type="date" defaultValue={user.birth?.slice(0, 10)} /></label><label className="wide">한 줄 소개<textarea name="self_intro" rows={3} defaultValue={user.self_intro} /></label></div><button className="primary-button submit-wide" disabled={busy}><Save /> 개인정보 저장</button></form>
      <div className="settings-side"><form className="content-card feature-form" onSubmit={changePassword}><h2>비밀번호 변경</h2><label>현재 비밀번호<input name="current_password" type="password" required /></label><label>새 비밀번호<input name="new_password" type="password" minLength={10} maxLength={128} title="문자와 숫자를 포함해 10자 이상 입력하세요 (최대 72바이트)" required /></label><label>새 비밀번호 확인<input name="confirm_password" type="password" minLength={10} maxLength={128} title="문자와 숫자를 포함해 10자 이상 입력하세요 (최대 72바이트)" required /></label><button className="primary-button submit-wide" disabled={busy}>변경하기</button></form>
        <section className="content-card"><h2>알림 설정</h2><PageState loading={preferenceResult.loading} error={preferenceResult.error} /><div className="toggle-list">{[['matching','팀/팀원 매칭'],['activity','활동 게시글'],['todo','활동 할 일'],['notice','공지사항']].map(([key,label]) => <button type="button" className={preferences[key] ? 'active' : ''} disabled={busy || preferenceResult.loading} aria-pressed={preferences[key]} onClick={() => { togglePreference(key); }} key={key}><span>{label}</span><i /></button>)}</div></section>
...
export function ActivityArchivePage() {
  ...
  return <><PageTitle title="지난 활동" description="완료한 경험을 미니포트폴리오로 다시 꺼내보세요." /><PageState loading={result.loading} error={result.error} empty={!result.loading && !result.data?.length ? '아직 완료된 활동이 없습니다.' : undefined} /><div className="archive-grid">{result.data?.map((item) => <Link className="archive-card" to={`/activity/portfolios/${item.portfolio_id}`} key={item.portfolio_id}><div className="archive-top"><span>{item.activity_type || '팀 활동'}</span><small>{item.archived_at?.slice(0, 10)} 저장</small></div><h3>{item.activity_name}</h3><p>{item.role || '역할 미정'} · {item.period || '기간 미정'}</p><div><CheckCircle2 /> 완료 작업 {item.completed_task_count || 0}개 <em>포트폴리오 보기</em></div></Link>)}</div></>;
}
