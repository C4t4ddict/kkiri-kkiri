...
export function TemplatesPage() {
  ...
  const remove = async (template: ApplicationTemplate) => {
    await api(`/api/application-templates/${template.template_id}`, { method: 'DELETE' });
  ...
  return <>
    ...
    {pendingDelete && <div className="modal-backdrop"><div className="web-modal"><div className="modal-head"><h2>지원서 삭제</h2><button onClick={() => setPendingDelete(null)}>×</button></div><p>“{pendingDelete.title}” 지원서를 삭제할까요?</p><div className="button-row"><button className="ghost-button" onClick={() => setPendingDelete(null)}>취소</button><button className="danger-button modal-danger" onClick={() => remove(pendingDelete)}>삭제</button></div></div></div>}
  </>;
}
