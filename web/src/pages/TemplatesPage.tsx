...
export function TemplatesPage() {
  ...
  const [pendingDelete, setPendingDelete] = useState<ApplicationTemplate | null>(null);
  ...
  const remove = async () => {
    ...
    await api(`/api/application-templates/${pendingDelete.template_id}`, { method: 'DELETE' });
  ...
  return <>
    ...
    {pendingDelete && <div className="modal-backdrop"><div className="web-modal confirm-modal"><div className="modal-head"><h2>지원서를 삭제할까요?</h2><button type="button" aria-label="닫기" onClick={() => setPendingDelete(null)}><X size={20} /></button></div><p><strong>{pendingDelete.title}</strong> 템플릿은 삭제 후 복구할 수 없습니다.</p><div className="button-row"><button className="ghost-button" onClick={() => setPendingDelete(null)}>취소</button><button className="danger-button" onClick={remove}>삭제</button></div></div></div>}
  </>;
}
