...
- `/curricula`: 기업별 과정 검색, 난이도 필터, 기간·학습량 비교
- `/curricula/:id`: 읽기 중심 과정 소개, 월간·주간·일일 학습 지도, 개인·팀 일정 미리보기
- `/activity`: 공모전과 기업 커리큘럼을 한 곳에서 관리하는 목표·달성률·공지 작업공간
- `/studio/curricula`: 운영자가 기업 정보와 계층형 목표를 작성하고 버전을 발행하는 화면

웹과 앱은 같은 커리큘럼 API를 사용하며, 개인·팀으로 추가된 과정은 모두 기존 `teams`, `team_members`, `todos` 기반 활동으로 표시됩니다.
