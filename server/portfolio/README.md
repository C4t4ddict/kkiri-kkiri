...
- `POST /users/:userId/past-activities/:portfolioId/images`: 포트폴리오 이미지 업로드
- `GET /users/:userId/past-activities/:portfolioId/pdf`: 한글 PDF 다운로드
- `POST /teams/:teamId/complete`: 팀장 수동 활동 마무리

사용자 편집값은 자동 생성 스냅샷과 분리된 `portfolio_edits` 테이블에 저장하므로 팀 활동 원본 기록은 유지됩니다. PDF는 Noto Sans KR과 PDFKit을 사용하며 편집한 표지 이미지, 활동 요약, 핵심 성과, 회고, 관련 링크와 월간·주간·일일 완료 작업 페이지로 구성됩니다.
