...
export default function ActivitySettingScreen() {
  ...
  const onToggle = (id: WidgetId) => {
    if (FIXED_VISIBLE_IDS.includes(id)) return;
    setPrefs(prev => toggleVisible(prev, id));
  };
  ...
  const Row = ({ item, idx, canMove }: { item: WidgetPref; idx: number; canMove: boolean }) => (
    <View style={s.row}>
      <Text style={[s.eye, item.visible ? s.eyeOn : s.eyeOff]} onPress={() => onToggle(item.id)}>
        {FIXED_VISIBLE_IDS.includes(item.id) ? '고정' : item.visible ? 'ON' : 'OFF'}
      </Text>
      <Text style={s.name}>
        {label(item.id)}
      </Text>
    ...
    <View style={s.wrap}>
      ...
      {hidden.length === 0 ? <Text style={s.empty}>숨김 항목 없음</Text> :
        <FlatList
          ...
          renderItem={({item}) =>
            <View style={s.row}>
              <Text style={[s.eye, s.eyeOff]} onPress={() => onToggle(item.id)}>OFF</Text>
...
const s = StyleSheet.create({
  ...
  eye: { width:36, textAlign:'center', fontSize:12, fontWeight:'700' },
