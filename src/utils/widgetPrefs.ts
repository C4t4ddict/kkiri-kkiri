...
function normalizePrefs(prefs: WidgetPref[]): WidgetPref[] {
  return prefs.map((pref) => (
    ALWAYS_VISIBLE_IDS.includes(pref.id)
      ? { ...pref, visible: true }
      : pref
  ));
}
...
function getStorage() {
  try {
    // Native module이 연결되지 않은 빌드에서도 활동 탭이 죽지 않도록 지연 로드합니다.
...
export async function loadWidgetPrefs(
  ...
  const raw = storage ? await storage.getItem(key) : memoryStore.get(key) ?? null;
...
export async function saveWidgetPrefs(
  ...
  const key = keyFor(teamId);
  const next = JSON.stringify(normalizePrefs(prefs));
  const storage = getStorage();
  if (storage) {
