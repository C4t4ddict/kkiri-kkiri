...
- 초기화 재실행 캡처: `/tmp/issue38-clean-login-2.png`
- 실제 API 제한 응답: `/tmp/issue38-fourth-blocked.json`
- 실제 API 완료 후 재추가 응답: `/tmp/issue38-fourth-retry.json`
- 동일 날짜에 미완료 기간 목표 3개를 생성한 뒤 네 번째 요청이 409로 차단되고 제한 개수 3이 반환됨을 확인함
- 첫 번째 기간 목표의 모든 일일 할 일을 완료한 뒤 네 번째 목표가 201로 생성되어 완료 시 슬롯이 즉시 해제됨을 확인함
- 검증용 기간 목표 12개 일일 할 일을 모두 삭제하고 잔여 데이터가 0건임을 확인함
- 기간 목표별 고정 레인을 배정해 시작일부터 종료일까지 같은 높이로 이어지고 활성 목표를 완료 목표보다 우선 표시함
- 앱 데이터를 초기화하고 API·Metro reverse를 다시 설정한 뒤 최신 로그인 화면이 정상 표시됨을 확인함
